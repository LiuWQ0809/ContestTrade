...
## Technical Highlights & Optimizations (技术亮点与优化)

为了提升生产环境的稳定性和成本效率，ContestTrade 进行了深度优化：

- **专业级日志与可视化**: 
  - 集成 `loguru` 与 `Rich`，提供精美的终端仪表盘。
  - **实时账户看板**: 支持展示“当日盈亏”、“当日收益率”及“累计收益率”。
  - **纯净日志**: 自动剥离 ANSI 转义字符，确保 `.log` 文件在任何编辑器中均清晰易读。
- **极致成本优化 (LLM Cache)**:
  - 引入**持久化磁盘缓存**机制（SHA-256），相同请求在多次运行间实现零 Token 消耗、零延迟响应。
  - **上下文压缩**: 智能截断过长的工具返回结果，保护推理质量并节省 Token。
- **账户感知决策**:
  - 研究智能体（Research Agent）具备实时账户感知能力，会根据可用现金和当前持仓优化研究重点。
  - **换仓逻辑**: 资金不足时，AI 会主动寻找并建议卖出表现较弱的头寸，以释放资金买入高确定性的新机会。
- **智能策略熔断**:
  - 余额不足（<1000且无持仓）时自动跳过分析，避免无效的 API 支出。
